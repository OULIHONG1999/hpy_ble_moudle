...
void user_client_report_data_callback(att_data_report_t *report_data)
{
    ...
    //          report_data->conn_handle,
    //          report_data->packet_type,
    //          report_data->blob_length);
...
/*
 * 功能：执行搜索操作处理
 * 说明：此函数静态内部调用，用于根据配置对已使用的操作句柄执行搜索处理，
 *      包括自动启用通知、指示和读取操作。
 */
/*************************************************************************************************/
...
static void __do_operate_search_handle(void)
{
    ...
    // 检查是否已配置GATT搜索或搜索UUID数量为0，若是则直接返回
    ...
    for (i = 0; i < __this->opt_handle_used_cnt; i++)
    {
        ...
        {
            // 若特性支持指示，且配置了自动启用CCC，则进行写操作以启用指示
            if (__this->gatt_search_config->auto_enable_ccc)
            {
                tmp_16 = 0x0002; // 固定值，用于写入CCC寄存器
        ...
        {
            // 对于可读特性，根据是否支持长读，选择不同的读取操作
            if (opt_hdl_pt->search_uuid->read_long_enable)
            {
                tmp_16 = 0x55A2; // 固定值，标识长读操作
            ...
            {
                tmp_16 = 0x55A1; // 固定值，用于普通读取操作
                log_info("read:%04x\n", opt_hdl_pt->value_handle);
                ble_comm_att_send_data(__this->client_search_handle, opt_hdl_pt->value_handle, &tmp_16, 2, ATT_OP_READ);
            }
        }
        else if (cur_opt_type & ATT_PROPERTY_WRITE_WITHOUT_RESPONSE)
        {
            log_info("write_without_resp:%04x\n", opt_hdl_pt->value_handle);
            ;
        }
        else
        {
            // 若当前特性类型未处理，则继续下一个循环迭代
            ;
        }
    }
}
 ...
 * 检查目标UUID是否匹配
 *
 * 本函数用于在搜索结果中查找与指定UUID匹配的服务和特征。如果找到匹配项，会进行一系列操作，包括分配操作句柄等。
 *
 * @param result_info 搜索结果信息的指针，包含服务和特征的UUID等信息。
 */
static void __check_target_uuid_match(search_result_t *result_info)
{
    ...
    // 遍历搜索配置中所有的UUID组，检查是否有匹配的服务UUID或特征UUID
    for (i = 0; i < __this->gatt_search_config->search_uuid_count; i++)
    {
        ...
        break; // 找到匹配的UUID，跳出循环
    }

    // 如果遍历完所有UUID组都没有找到匹配项，则直接返回
...
void user_client_report_search_result(search_result_t *result_info)
{
    ...
    log_info("{charactc, uuid16:%04x,index=%d,handle:%04x~%04x,value_handle=%04x}\n",
             result_info->characteristic.uuid16,
             result_info->characteristic_index,
...
static bool __check_device_is_match(u8 event_type, u8 info_type, u8 *data, int size, client_match_cfg_t **output_match_devices)
{
    ...
    // 当profile未开启搜索或者没有需要搜索的设备时，就返回false
    ...
    for (i = 0; i < __this->gatt_search_config->match_devices_count; i++)
    {
        ...
        // 1. 将传入的匹配模式跟设定的匹配模式进行对比 两种mode是否相同
        // 2. 需要对比的数据长度进行对比 设定的长度和搜索到的数据的长度
        if (cfg->create_conn_mode == conn_mode && size == cfg->compare_data_len)
        {
            log_info("dev check\n");
            log_info_hexdump(data, size); // 目标设备发送来的的匹配信息
            log_info_hexdump(cfg->compare_data, size);

            // 判断设定的数据与收索到的设备数据进行对比
...
static bool __resolve_adv_report(adv_report_t *report_pt, u16 len)
{
    ...
    // printf("##################################");
#if 1
// 用户自定义设备匹配功能 根据需求可以按名字匹配或按地址匹配
#include "ble_drive.h"
    extern u8 user_check_device_match(adv_report_t * report_pt, u16 len);
    u8 is_match = user_check_device_match(report_pt, len);
    // 如果匹配成功，就直接跳转连接过程
    if (is_match)
    {
        find_remoter = 1;
