...
static int cetl_client_event_packet_handler(int event, u8 *packet, u16 size, u8 *ext_param)
{
    ...
    case GATT_COMM_EVENT_GATT_DATA_REPORT:
    {
        ...
        extern void (*receive_callback)(u8 *data, u8 len, u8 handle);

        receive_callback(report_data->blob, report_data->blob_length, report_data->conn_handle);
    ...
    case GATT_COMM_EVENT_CONNECTION_COMPLETE:
        ...
        printf("GATT_COMM_EVENT_CONNECTION_COMPLETE");
        put_buf(packet, size);
        put_buf(ext_param, 20);
        ...
        // TODO: 将连接的设备信息保存到ble_device_info中
        set_ble_device_info(ext_param, 15);
        // 清除匹配模式
        clear_match_mode();
    ...
    case GATT_COMM_EVENT_DISCONNECT_COMPLETE:
        ...
        // TODO: 设备断开，清除设备相关信息
        extern void device_lost(u8 handle);
        device_lost(little_endian_read_16(packet, 0));
    ...
    {
        printf("GATT_COMM_EVENT_GATT_SEARCH_PROFILE_COMPLETE");
        put_buf(packet, size);
...
void att_send_data(u8 *data, u8 len, u8 handle)
{

    // 检查是否已建立连接，数据是否可以发送，以及是否已配置CCC
    if (handle &&
        ble_comm_att_check_send(handle, len))
    {
        // 发送数据
        ble_comm_att_send_data(handle, ATT_CHARACTERISTIC_ae01_01_VALUE_HANDLE, data, len, ATT_OP_WRITE_WITHOUT_RESPOND);
    }
    else
    {
        // 放弃发送数据并记录日志
